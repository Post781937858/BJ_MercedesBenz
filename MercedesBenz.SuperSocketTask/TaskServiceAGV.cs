...
{
    public class TaskServiceAGV : SuperSocketBaseTask
    {

        public override void MessageDispose(ServerSession session, ServerRequestInfo requestInfo)
        {
            List<byte[]> byteList = BytePackagedis.AnalysisByte(requestInfo.BodyByte);
            foreach (var message in byteList)
            {
                ...
                if (message[7] == 0x10)
                {
                    ...
                    info.navigationText = info.navigationStr();
                    info.ThisStation = (message[19] << 8 | message[20]); //当前战点
                    info.TargetStation = (message[21] << 8 | message[22]); //目标战点
                    info.taskStatus = (Models.TaskStatus)message[23]; //上/下料完成状态
                    info.taskStatusText = info.UpdownStatus();
                    ...
                    info.updateDateInfoTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                    if (SystemTaskDispose.agvInfoList.ContainsKey(info.agvNumber))
                    {
                        if (info.ThisStation == 0)
                        {
                            info.ThisStation = SystemTaskDispose.agvInfoList[info.agvNumber].ThisStation;
                        }
                        info.CarIptype = SystemTaskDispose.agvInfoList[info.agvNumber].CarIptype;
                        info.UpdateStopDateTime = SystemTaskDispose.agvInfoList[info.agvNumber].UpdateStopDateTime;
                        info.UpdateDateTime = UTC.ConvertDateTimeLong(DateTime.Now);
                        info.StopRunStatus = SystemTaskDispose.agvInfoList[info.agvNumber].StopRunStatus;
                        SystemTaskDispose.agvInfoList[info.agvNumber] = info;
                    }
                    if (SystemTaskDispose.agvInfoList.ContainsKey(info.agvNumber))
                    {
                        var agvinfo = SystemTaskDispose.agvInfoList[info.agvNumber];
                        var mesg = GroupMessage.responsePLCInfo(message);
                        session.Send(mesg,0, mesg.Length);
                      //SystemTaskDispose.SendServer(IPType.agvserver, mesg, agvinfo.CarIptype);
                    }
                }
                else if (message[7] == 0x03)
                {
                    ...
                    var agvInfo = GetagvInfo(session);
                    if (agvInfo != null)
                    {
                        ...
                        if (orderInfo == null)
                        {
                            ...
                            {
                                var DoorInfo = SystemTaskDispose.DoorInfoArray[DoorType.Out];
                            ...
                            {
                                var DoorInfo = SystemTaskDispose.DoorInfoArray[DoorType.In];
                                if (DoorInfo.DoorStatus == DoorStatus.Close)
                                {
                                    agvInfo.agvRunStop = 1;
                                }
                            ...
                            //重载agv任务调度
                            if (agvInfo.CarIptype == IPType.weight)
                            {
                                //待执行订单
                                ...
                                if (NotexecuteTask.Count() > 0)
                                {
                                    ...
                                    if (awaitexecuteTask.order_type == OrderTaskType.In)
                                    {
                                        ...
                                        foreach (var item in bufferList)
                                        {
                                            ...
                                            {
                                                SystemTaskDatabase.Instance.UpdatemagicCarNo(1, awaitexecuteTask.order_ordernumber, agvNumber);
                                            }
                                            break;
                                        }
                                    }
                                    ...
                                    {
                                        var bufferList = bufferStatus.Where(p => p.Buff_Site == awaitexecuteTask.order_getSite.ToInt());
                                        foreach (var item in bufferList)
                                        {
                                            if (item.Buff_status == 1)
                                            {
                                    ...
                                    if (Originsite != null)
                                    {
                                        ...
                                        StationList.ForEach(p =>
                                        {
                                            if (p.station_agvSite == agvInfo.ThisStation && p.station_number != 2)
                                            {
                                                ...
                                                {
                                                    SendTaskByte(agvInfo, new OrderTask(), message, session);
                                                }
                                                else
                                                {
                                                    SystemTaskDatabase.Instance.InsertOriginTask(new OrderTask() { order_carno = agvNumber, order_getSite = Originsite.site_agvsite.ToString() });
                                        ...
                                        //else
                                        //{
                                        //    var await = StationList.FirstOrDefault(p => p.station_number == 2);
                                        //    if (await != null)
                                        //    {
                                        //        if (agvInfo.ThisStation == await.station_agvSite)
                                        //        {
                                        //            if (agvInfo.UpdateStopDateTime == 0)
                                        //            {
                                        //                //站点172默认等待60分钟(防止分拣未完成需做回库处理)
                                        //                agvInfo.UpdateStopDateTime = UTC.ConvertDateTimeLong(DateTime.Now);
                                        //            }
                                        //            var time = agvInfo.UpdateDateTime - agvInfo.UpdateStopDateTime;
                                        //            if (agvInfo.UpdateDateTime - agvInfo.UpdateStopDateTime > 600)
                        ...
                        else
                        {
                            if (orderInfo.order_magic == 1) //阶段一前往取货点
                            {
                                ...
                                {
                                    SendTaskByte(agvInfo, orderInfo, message, session);
                                }
                            }
        ...
        private bool InIsRun()
        {
            ...
            foreach (var agvNumber in SystemTaskDispose.agvInfoList.Keys)
            {
                ...
                if (info.location_X >= 14 && info.location_X <= 50 && info.location_Y >= 24 && info.location_Y <= 50)
                {
                    Run = true;
                }
            }
        ...
        private agvInfo GetagvInfo(ServerSession session)
        {
            string IpAddress = session.RemoteEndPoint.Address.ToString();
            List<ServiceModel> services = SystemConfiguration.Servicecfig();
            ServiceModel modelConfig = services.FirstOrDefault(p => p.IP == IpAddress);
            if (modelConfig == null)
            {
                throw new Exception($"无法找到{IpAddress}:{session.RemoteEndPoint.Port} 对应配置参数，请检查ServiceConfig.json是否配置错误");
            }
            return SystemTaskDispose.agvInfoList.Values.FirstOrDefault(p => p.CarIptype == modelConfig.type);
        }
        ...
        private void SendTaskByte(agvInfo agvInfo, OrderTask orderInfo, byte[] message, ServerSession session)
        {
            byte[] awaitTask;
            ...
            session.Send(awaitTask, 0, awaitTask.Length);
            //SystemTaskDispose.SendServer(IPType.agvserver, awaitTask, agvInfo.CarIptype);
        }
        ...
        public override void ServerTaskRun()
        {
            ...
            foreach (var agvInfoitem in SystemTaskDispose.agvInfoList.Values)
            {
                ...
                carInfo.Info_errormes = ErrorCodeMessage(agvInfoitem.ErrorCode, out bool Iserror);
